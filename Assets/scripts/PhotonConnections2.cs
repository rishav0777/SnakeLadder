...
public class PhotonConnections2 : MonoBehaviourPunCallbacks
{
    ...
    {
        stime=Time.time;
        DataSaver2.Instance.SetMode(1);
        Debug.Log("mode "+DataSaver2.Instance.GetMode());
    ...
    private void Update()
    {
        if (gameFlag)
        {
            ...
            else if (timer.GetTimer() <5f)
            {
                DataSaver2.Instance.SetMode(2);
                StartCoroutine(JoinGame());
            }
        }
        else if (timer.GetTimer() < 5f)
